...
# 1. Carregamento dos Dados (Exemplo com os 24 meses da Linha 1)
df = pd.read_csv('dataset/trabalho_demanda.csv')
demandas_L1 = df['L1'].tolist()

print("--- INICIANDO O TORNEIO DE TÉCNICAS: LINHA 1 ---\n")
demandas_L1 = tratar_anomalias_demanda(demandas_L1)
# 2. Aplicação das 3 Técnicas Escolhidas (Apenas nos 24 meses históricos)
# Técnica 1: Regressão Linear
resultado_regressao = regressao_linear(demandas_L1)
previsoes_regressao = resultado_regressao["previsoes"]

# Técnica 2: Holt Duplo
resultado_holt = modelo_holt(demandas_L1)
previsoes_holt = resultado_holt["previsoes"]

# Técnica 3: Média Exponencial Móvel
previsoes_mem = media_exponencial_movel(demandas_L1, alpha=0.4)

# 3. Cálculo das Métricas de Erro usando o seu arquivo independente
erros_regressao = calcular_erros(demandas_L1, previsoes_regressao)
erros_holt = calcular_erros(demandas_L1, previsoes_holt)
erros_mem = calcular_erros(demandas_L1, previsoes_mem)
